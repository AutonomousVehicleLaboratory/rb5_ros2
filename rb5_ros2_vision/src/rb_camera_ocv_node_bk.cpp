...
int main(int argc, char *argv[]){
  ...
  auto node = rclcpp::Node::make_shared("rb_camera");

  private_nh.param("camera_id", _camera_id, 0);
  RCLCPP_INFO(node->get_logger(), "camera_id: %d", _camera_id);
  private_nh.param("frame_rate", _frame_rate, 30);
  RCLCPP_INFO(node->get_logger(), "frame_rate: %d", _frame_rate);
  private_nh.param("width", _width, 1920);
  RCLCPP_INFO(node->get_logger(), "width: %d", _width);
  private_nh.param("height", _height, 1080);
  RCLCPP_INFO(node->get_logger(), "height: %d", _height);
  private_nh.param<std::string>("input_format", _input_format, "NV12");
  RCLCPP_INFO(node->get_logger(), "input_format: %s", _input_format.c_str());
  private_nh.param<std::string>("output_format", _output_format, "RGB");
  RCLCPP_INFO(node->get_logger(), "output_format: %s", _output_format.c_str());
  private_nh.param<std::string>("topic_name", _topic_name, std::string("camera")+std::to_string(_camera_id));
  RCLCPP_INFO(node->get_logger(), "topic_name: %s", _topic_name.c_str());
  private_nh.param("image_compress", _image_compress, true);
  RCLCPP_INFO(node->get_logger(), "image_compress: %d", _image_compress);

  auto cam_pub = node->create_publisher<sensor_msgs::msg::Image>(_topic_name.c_str(), 10);
  // if (_image_compress) {
